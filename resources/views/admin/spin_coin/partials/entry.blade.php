...
@for($i = 1; $i <= 8; $i++)
    <div class="row">
        <div class="col-sm-9">
            <div class="form-group">
                <label for="key_{{ $i }}">Giá trị {{ $i }}</label>
                <input type="number" class="form-control" name="key_{{ $i }}" id="key_{{ $i }}" placeholder="Giá trị {{ $i }}" value="{{ isset($item->properties[$i]) ? $item->properties[$i] : '' }}">
            </div>
        </div>
        <div class="col-sm-3">
            <div class="form-group">
